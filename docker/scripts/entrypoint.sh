...
STACK_SCHEME=${STACK_SCHEME:-http}
...
COUNT=0
DB_CONFIG_FILE=/cartodb/config/database.yml
APP_CONFIG_FILE=/cartodb/config/app_config.yml
...
HOME_CONTROLLER="/cartodb/app/controllers/home_controller.rb"
...
MXVALID_INIT_FILE="/cartodb/config/initializers/email_address.rb"
...
echo "--- Starting varnish on http ${VARNISH_HTTP_PORT}..."
/opt/varnish/sbin/varnishd -a :${VARNISH_HTTP_PORT} -s malloc,256m -f /etc/varnish.vcl

echo "--- Starting nginx via service..."
service nginx start

echo "--- Starting Windshaft node app..."
cd /Windshaft-cartodb
node app.js $CARTO_ENV &

echo "--- Starting SQL API node app..."
cd /CartoDB-SQL-API
node app.js $CARTO_ENV &
...
echo "--- Starting Resque process, logging to /cartodb/resque.log..."
...
bundle exec thin start --threaded -p $CARTODB_LISTEN_PORT -a 0.0.0.0 --threadpool-size 5 > rails.log 2>&1 &
